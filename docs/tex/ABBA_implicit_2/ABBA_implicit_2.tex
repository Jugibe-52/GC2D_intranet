...
\fancyhead[R]{\small\color{gray}Implicit 2: simultaneous equation (21)}
\fancyfoot[C]{\thepage}
...
\begin{center}
  {\LARGE\bfseries\color{blue}Implicit ABBA 2 with Hairer's Symmetric Projection}\par
  \vspace{0.35em}
...
\section{Purpose and relation to implicit ABBA 1}

Implicit ABBA 2 computes the same exact projected map as implicit ABBA 1 but
uses a different nonlinear system. Implicit ABBA 1 eliminates the duplicated
output state and solves only for the two-component multiplier. Implicit ABBA 2
retains both the four-component duplicated output and the multiplier as Newton
...
\]
Implicit ABBA 2 retains the diagonal output $\Y_{n+1}$ as an additional
unknown and imposes
\[
 d(\Y_{n+1},\mu)
 :=\Y_{n+1}-N\mu
...
\[
 ...
 D_\mu g=0.
\]
Therefore, at Newton iterate $(\Y_{n+1}^{(k)},\mu^{(k)})$, implicit ABBA 2
solves
\[
 \boxed{
 \begin{pmatrix}
...
\section{Schur complement and equivalence to implicit ABBA 1}
...
\]
This is exactly the reduced Newton correction used by implicit ABBA 1. Hence both
algorithms generate the same multiplier iterates in exact arithmetic when they start
from the same $\mu^{(0)}$, even though implicit ABBA 2 also updates the duplicated
output unknown.
...
\]
Thus implicit ABBA 2 is the same symmetric and symplectic physical map as implicit
ABBA 1 at the exact root. In finite arithmetic, measured reversibility and symplecticity
