...
common analytic expression.  The HDF5 evaluator also clips coordinates at the
sampled rectangle.  Unless the interior derivative vanishes at the boundary,
the constant clipped extension loses differentiability there.
...
for the spline-and-clipping runtime representation.  Finite-smoothness
...
\begin{enumerate}
 ...
 \item prove a finite-regularity theorem with an explicit $N$ supported by the
       chosen spline degree, while keeping every stage away from the clipped
       boundary.
\end{enumerate}
...
\begin{itemize}
 ...
 \item \textbf{Not covered by the exponential theorem as currently represented:}
       spline-knot crossings, clipped-boundary crossings, finite nonlinear
...
the potential, a domain separated from clipping, conditioning bounds for the
implicit equations, and explicit control of nonlinear and round-off errors.
