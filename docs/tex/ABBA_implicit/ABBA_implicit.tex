...
\fancyhead[R]{\small\color{gray}Projection and constant Jacobian}
\fancyfoot[C]{\thepage}
...
\begin{center}
  {\LARGE\bfseries\color{blue}ABBA Method with Hairer's Symmetric Projection}\par
  \vspace{0.35em}
...
\subsubsection*{Detailed construction of equation (23)}

Expression (23) combines in a single function the two conditions required for the two
...
\[
 ...
 =(\mu,-\mu)\in\R^4.
\]
Therefore, the ABBA argument in (23) is the pre-corrected state
\[
 Ez+N\mu=(z+\mu,z-\mu).
\]
...
\[
 GN\mu=GG^T\mu=2\mu.
\]
This calculation explains the term $2\mu$ in (23): it is neither an approximation nor an
...
$u^{(2)}=(u_X^{(2)},u_Y^{(2)})^T$,
$v^{(2)}=(v_X^{(2)},v_Y^{(2)})^T$ y
$\mu=(\mu_X,\mu_Y)^T$, the nonlinear system (23) is
\[
 \mathcal R(z,\mu)=
 \begin{pmatrix}
...
\]
Thus, (23) correctly defines a map
\[
...
\[
 ...
 =GEz+GN\mu+2\mu.
\]
Ahora bien, $GE=0$ y $GN=2\Id_2$, de modo que
\[
 \mathcal R(z,\mu)=4\mu.
\]
...
\]
Al derivar (22) con respecto a $z$ se obtiene
\[
 \boxed{
...
Equation (25) and the symplecticity of ABBA in (8) imply
\begin{align}
 ...
 &\quad=(E+NM)^T
          ...
          (E+NM) \notag\\
 &\quad=(E+NM)^T\Om(E+NM). \tag{27}
\end{align}
The last equality is exactly where symplecticity of the duplicated ABBA map is used.
Thus, (27) states that $D\PhiAB$ preserves $\Om$ between the initial and final lifted perturbations.

Using the four identities in (26), the initial side becomes
\begin{align}
 (E+NM)^T\Om(E+NM)
...
\begin{align}
 ...
 &=2J_\Psi^TJ_0J_\Psi-2M^TJ_0M. \tag{29}
\end{align}

Sustituyendo (28) y (29) en (27), obtenemos
\[
...
and cancels. Hence,
\[
 \boxed{J_\Psi^TJ_0J_\Psi=J_0.} \tag{31}
\]
...
\begin{enumerate}
 ...
 \item $D_\mu\mathcal R$ es invertible, de modo que $\mu_h(z)$ es diferenciable;
 \item the nonlinear projection equation is solved exactly.
\end{enumerate}
The constant matrix used to compute the nonlinear correction changes only the iterative
solver. If the iteration converges to the exact root, it does not change the projected map
or the proof above. With a finite stopping tolerance, symplecticity is preserved only up
to the defect introduced by that tolerance.
