...
The public ABBA family contains exactly four numerical-method classes:
\begin{itemize}
...
method classes. The three implicit methods expose two nonlinear selectors and
a constrained state/energy strategy:
...
\texttt{track\_energy}
  & \texttt{False}; \texttt{True}; fully extended execution always resolves
    to \texttt{True} \\
\bottomrule
\end{tabular}
\end{center}

The three normalized state/energy strategies are physical without tracking,
physical with tracking, and fully extended with tracking.  ABBA2 and ABBA6
therefore admit $2\times2\times3=12$ canonical configurations each.
ABBA4 adds two projection placements and admits 24.
\texttt{ABBA2Midpoint} has no nonlinear residual and supports the same three
strategies.  The four public classes consequently represent
$12+24+12+3=51$ canonical configurations.

The formulation and solver choices are global within a composed step. For
\texttt{ABBA4Implicit}, the per-map placement applies the same configuration
to all three signed maps independently; the exterior placement surrounds the
complete unprojected triple jump with one projection. All seven signed maps of
\texttt{ABBA6Implicit} use the same configuration.
...
\begin{itemize}
 ...
 \item \texttt{track\_energy}: \texttt{False}.
\end{itemize}
...
\section{State, energy tracking, and dimensional conventions}

The \texttt{state\_extension} selector chooses which variables are duplicated,
while \texttt{track\_energy} optionally advances conjugate momentum beside a
physical solve.  For one guiding-center particle, the complete dimensional
convention is:
...
\toprule
Extension and tracking & Accepted state & Splitting state & Reduced unknown & Simultaneous unknown \\
\midrule
\texttt{physical}, off
  & $z\in\R^2$
  & $(u,v)\in\R^4$
...
and $6N$, respectively, whether tracking is disabled or enabled.  Tracking
adds $N$ conjugate momenta to the fixed-grid workspace, not to any state or
unknown in the table.  Fully extended execution currently requires $N=1$.

Physical observers receive the closed map $z\mapsto z_{n+1}$, independently
of energy tracking.  The optional momentum history remains in diagnostics and
does not enter observer states or Jacobians.  Fully extended observers instead
...
Physical tracking reuses the four accepted ABBA stage states to advance one
normalized conjugate value $\kappa=k/2$ per particle.  Because this momentum
update is triangular, it does not feed back into $z$ and the physical
trajectory agrees with the untracked configuration.  The temporary $\R^6$
nonlinear unknown of the simultaneous formulation is unrelated to this
auxiliary momentum block.  Diagnostics store it as
\texttt{extended\_momentum}, together with
\texttt{extended\_momentum\_normalization} and the scalar maximum drift
\texttt{energy\_error}; time is the ordinary solution time.
...
$\R^{12}$ nonlinear workspace shown in the table.  Fully extended execution
always resolves \texttt{track\_energy} to \texttt{True} and reports direct-$k$
energy diagnostics.  Every public ABBA class selects these strategies through
\texttt{state\_extension} and \texttt{track\_energy}; no separate extension
method class is exposed.
