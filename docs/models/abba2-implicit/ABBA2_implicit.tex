...
The public implementation is \texttt{ABBA2Implicit}.  Its
\texttt{projection\_formulation} field selects one of two algebraically equivalent
projection solvers:
\begin{itemize}
 \item \texttt{reduced\_multiplier};
 \item \texttt{simultaneous\_state\_multiplier}.
\end{itemize}
Both formulations traverse the same duplicated ABBA map and define the same
accepted physical map, up to the nonlinear stopping tolerance.

This note derives the reduced multiplier formulation.  Its nonlinear unknown is
$\mu\in\R^2$ per particle and its exact Newton matrix is $2\times2$.  The
simultaneous formulation instead solves for $(u_f,v_f,\mu)\in\R^6$ per particle
with a $6\times6$ Newton block.  That six-dimensional vector is only an algebraic
solver workspace; it is not a trajectory state or a separate numerical method.
...
\section{Time extensions and dimensional conventions}

The implementation keeps two time-extension families separate because they duplicate
different variables.  For one guiding-center particle, the relevant spaces are:
...
\toprule
Method & Accepted internal state & Base splitting state & Momentum convention \\
\midrule
\texttt{ABBA2SharedTime}\allowbreak\texttt{ExtendedImplicit}
  & $(z,t,\kappa)\in\R^4$
  & $(u,v,t,k)\in\R^6$
  & $\kappa=k/2$ \\
\texttt{ABBA2Fully}\allowbreak\texttt{ExtendedImplicit}
  & $(z,t,k)\in\R^4$
  & $(Z_1,Z_2)\in\R^8$
  & direct $k$ \\
\texttt{ABBA4Fully}\allowbreak\texttt{ExtendedImplicit}
  & $(z,t,k)\in\R^4$
  & $(Z_1,Z_2)\in\R^8$
  & direct $k$ \\
\bottomrule
...
Here $Z=(z,t,k)$.  The shared-time lift duplicates only the physical state and
shares one $(t,k)$ pair.  Its accepted conjugate variable is normalized as
$\kappa=k/2$, and its physical trajectory is identical to that of
\texttt{ABBA2Implicit} with the same projection formulation.  Its $\R^6$ object
is therefore a genuine splitting state, unlike the temporary $\R^6$ unknown of
the simultaneous nonlinear formulation.

The fully extended methods duplicate the complete autonomous state, operate on
$(Z_1,Z_2)\in\R^8$, and advance $k$ directly.  Their full-state projection defines
a different numerical map; it is not merely another solver formulation of
\texttt{ABBA2Implicit}.  All three extension classes currently require exactly one
particle so that the displayed dimensions are literal.
