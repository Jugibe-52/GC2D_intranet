# DOP853 adaptive integration architecture

[Editable diagram](dop853-simulation-architecture.puml) · [Scalable diagram](dop853-simulation-architecture.svg)

![DOP853 architecture](dop853-simulation-architecture.png)

`DOP853` is a public numerical method using the same preparation, coordinator,
collector and result validation as the fixed-step methods. Its backend is
SciPy's `DOP853` solver. The six-phase diagram retains the detailed BM4 structure.

```python
from simulation import DOP853, SimulationRequest, simulate

solution = simulate(
    problem,
    DOP853(relative_tolerance=1e-10, absolute_tolerance=1e-12),
...
Explicit high-order Runge--Kutta; embedded error estimates select/reject trial intervals. SciPy DOP853 preserves its stage workspace. Accepted dense output is order seven. No Jacobian or LU factorization is used.
...
method = DOP853(dense_output=True, step_observer=trajectory)
