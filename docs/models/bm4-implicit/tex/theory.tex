...
After convergence, passive energy quadrature evaluates $-\partial_t H$ at the
24 shear inputs retained from the accepted residual, with their original signed
durations. It repeats no spatial map or coupling and performs no new nonlinear
solve. Trial traces are discarded; energy evaluations remain outside physical
solver counters.
...
The public class is in \path{src/methods/extended/bm4.py}.
Its twelve-stage recipe is defined in \texttt{composition.py}, and its
reduced equation uses \texttt{projection.py}. See \texttt{implicit-reduced.tex}
for the projection derivation. The former BM4 import adapters have been removed.
