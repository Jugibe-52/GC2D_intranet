...
This placement of \texttt{ABBA4Implicit} exposes the same controls as its
per-map placement: two projection formulations, Newton or Broyden, physical or
...
\section{Difference from the per-map placement}

Both \texttt{ABBA4Implicit} placements use $(\gamma,\delta,\gamma)$, but
\texttt{after\_each\_abba\_map} solves three projections and re-embeds a
physical state between signed factors.  The
\texttt{around\_complete\_composition} placement solves one projection around
the full unprojected composition.  Since projection and factor evolution do
not commute, the placements define different numerical maps rather than two
costs for the same map.
...
The public \texttt{ABBA4Implicit} placement selector is in
\path{src/simulation/methods/abba/order4_implicit.py}; the exterior-projection
residual and private kernel remain in
...
dimensions, diagnostics, and runtime flow for both placements.
