...
def main():
    ...
    status = {'run_id':run, 'status':'running', 'started_utc':utc_now(),
              'particles':48, 'workers':16, 'cycles':5000, 'steps_per_cycle':20}
    ...
    def sync_progress():
        ...
        progress = {'run_id':run, 'updated_utc':utc_now(), 'worker_completed_steps':completed,
                    'total_steps_per_worker':100000, 'minimum_completed_cycles':min(completed)/20,
                    'maximum_completed_cycles':max(completed)/20,
                    'average_percent':sum(completed)/16/1000,
    ...
    try:
        ...
        assert (meta['particle_count'],meta['cycles'],meta['steps_per_cycle'],meta['process_count']) == (48,5000,20,16)
        assert meta['newton_history_recorded'] and not meta['reference_computed']
        assert len(store.uploaded) == 16 * 200
