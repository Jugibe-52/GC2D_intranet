"""Restartable fixed-grid orchestration of the frozen implicit BM4 step."""
...
def calculate_checkpointed_group(indices, initial_xy, settings):
    ...
    contract = {
        'schema': 1, 'settings': {k: settings[k] for k in (
            'rho', 'coupling_frequency', 'newton_atol', 'newton_rtol',
    ...
    with threadpool_limits(limits=1):
        ...
        while completed < n:
            ...
            mu = np.empty(length)
            with observe_newton(length, progress_every=0) as history:
            ...
            atomic_json(file.with_suffix('.json'), {
                ...
                'particle_ids': (indices + 1).tolist(), 'pid': os.getpid()})
            parts.append(part)
            completed = end
            new_chunks += 1
    ...
    return {
        ...
        'record': {'pid': os.getpid(), 'particle_ids': (indices + 1).tolist(),
                   ...
                   'maximum_residual_to_tolerance': float(np.max(
                       diagnostics['nonlinear_residual_norms'] / diagnostics['nonlinear_tolerances']))},
    }
