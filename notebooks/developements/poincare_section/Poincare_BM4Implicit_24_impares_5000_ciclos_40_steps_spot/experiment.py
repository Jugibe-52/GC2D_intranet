...
def compute_and_save(parameters, run_id, validation_run=False):
    ...
    assert workers == 16 and n >= workers and cycles > 0 and spc > 0
    implicit = p['method'] == 'BM4Implicit'
    assert implicit or p['method'] == 'BM4Midpoint'
    steps = cycles * spc
    ...
    meta = {**params, 'run_id':run_id,'schema_version':1,'complete_steps':steps,
            ...
            'partition':'round-robin over selected particle IDs; independent Newton stopping per group',
            ...
            'checkpoint_driver_sha256':digest(ROOT/'checkpoint_calculation.py'),
