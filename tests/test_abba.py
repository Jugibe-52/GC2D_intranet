...
class SymmetricProjectedABBATests(unittest.TestCase):
	...
	def test_non_autonomous_stages_use_both_step_endpoints(self) -> None:
		...
		solution = simulate(
			...
			SymmetricProjectedABBA(step_observer=lambda _step: None),
	...
	def test_observation_and_diagnostics_ignore_shadow_steps(self) -> None:
		...
		observed = simulate(
			problem,
			SymmetricProjectedABBA(step_observer=events.append),
			SimulationRequest.uniform(
				t_span=(0.0, 0.05),
				max_step=0.02,
				sample_count=11,
	...
	def test_zero_field_is_identity_and_invalid_inputs_fail_early(self) -> None:
		...
		with self.assertRaises(ValueError):
			SymmetricProjectedABBA(newton_absolute_tolerance=True)
		with self.assertRaises(ValueError):
			SymmetricProjectedABBA(newton_relative_tolerance=0.0)
		with self.assertRaises(ValueError):
			SymmetricProjectedABBA(newton_max_iterations=0)
...
class ABBASymplecticityStudyTests(unittest.TestCase):
	...
	def test_short_study_returns_solver_and_symplecticity_diagnostics(self) -> None:
		...
		self.assertEqual(result.method_name, "SymmetricProjectedABBA")
