"""Tests for the aligned seven-variant trajectory comparison."""
...
class TenMethodTrajectoryComparisonTests(unittest.TestCase):
	...
	def test_all_variants_share_the_problem_and_time_grid(self) -> None:
		...
		self.assertEqual(len(result.solutions), 7)
		...
		self.assertEqual(len(result.trajectory_difference_summaries()), 21)
		self.assertEqual(len(result.runtime_summaries()), 7)
		self.assertEqual(len(result.nonlinear_work_summaries()), 6)
		self.assertEqual(len(result.implicit_solutions), 6)
		self.assertEqual(
		...
		self.assertEqual(len(difference_axis.texts), 49)
		...
		self.assertEqual(len(runtime_axis.patches), 7)
		...
		self.assertEqual(len(animation._func(1)), 15)
		animation._draw_was_started = True
		plt.close(animation._fig)
		eight_solutions = dict(result.solutions)
		eight_solutions["Additional method"] = next(iter(result.solutions.values()))
		eight_animation = animate_trajectory_points(
			result.effective_potential,
			eight_solutions,
			frames=2,
			interval=10,
		)
		self.assertEqual(len(eight_animation._func(1)), 17)
		eight_animation._draw_was_started = True
		plt.close(eight_animation._fig)
