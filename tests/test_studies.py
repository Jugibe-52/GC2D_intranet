...
def small_potential_config() -> RandomPotentialConfig:
	"""Return a deterministic field small enough for study contract tests."""
...
class RK4SymplecticityStudyTests(unittest.TestCase):
...
class ABBASymplecticityStudyTests(unittest.TestCase):
	...
	def test_short_abba_study_reports_a_numerical_floor(self) -> None:
		...
		with self.assertRaises(TypeError):
			first_solution.diagnostics["newton_residual_norms"] = np.asarray([])  # type: ignore[index]
