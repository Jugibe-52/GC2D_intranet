"""Generalized-energy convergence workflows for GC experiments."""
...
def run_generalized_energy_comparison(
	...
	*,
	...
	if not isinstance(trajectory, TrajectoryGC):
		raise TypeError("`trajectory` must be a TrajectoryGC instance.")
	if not isinstance(config, GeneralizedEnergyConfig):
		raise TypeError("`config` must be a GeneralizedEnergyConfig instance.")
	initial_state = trajectory.initial_state
	if initial_state is None or trajectory.particle_count(initial_state) != 1:
		raise ValueError(
