...
def _solve_method_step(
	...
	if method._variant == "abba4":
		...
		for coefficient in _ABBA4_COEFFICIENTS:
			...
			result = _solve_full_projection(
				...
				context=(
					f"ABBA4_implicit2 substep at t={current[2]:.16g} "
	...
	if method._variant == "bm4":
		base_map = _bm4_base_map(
			dynamics,
			duration,
			frequency=method.coupling_frequency,
		)
...
def _integrate_fully_extended(
	...
	if not isinstance(problem.dynamics, GuidingCenterDynamics):
		raise TypeError(f"{type(method).__name__} requires GuidingCenterDynamics.")
	physical_initial = np.asarray(problem.initial_state, dtype=float)
	if physical_initial.shape != (2,):
		raise ValueError(f"{type(method).__name__} requires exactly one GC particle.")
	...
	return IntegrationData(
		...
		diagnostics={
			...
			"projection_solver_formulation": (
				"fully_duplicated_z_t_k_projection"
			),
...
class _FullyExtendedImplicitMethod:
	...
	def __post_init__(self) -> None:
		"""Validate nonlinear and coupling controls."""
		object.__setattr__(
			self,
