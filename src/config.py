...
from classes import GridPotential, System, Trajectory, create_system, create_trajectory
...
DEFAULT_FOURIER_CONFIG = (
	CONF_DIR
	/ DEFAULT_CONFIG_SURFACE
	/ "fourier"
	/ DEFAULT_CONFIG_GROUP
	/ f"{DEFAULT_CONFIG_VERSION}.json"
)
...
SOLVER_PARAM_KEYS = {"TimeStep", "ode_solver", "CheckEnergy"}
OUTPUT_PARAM_KEYS = {"plot", "data", "wrap", "extension", "dpi"}
...
def config_path(
	...
	"""Return the default path for one configuration family."""
	kind = Path(name).stem
...
def _version_payload(
	...
	if selected not in versions:
		...
		raise ConfigError(
			f"Version {selected!r} is not defined in {path}. Available: {available}."
		)
...
def _merge_config_block(
	...
	for key, value in override.items():
		if key != block_name:
			merged[key] = value
	return merged, cast(ParameterMap, block.copy())


def _normalize_fourier_params(params: ParameterMap) -> FourierParams:
...
def _apply_fourier_output(
	defaults: ParameterMap,
	output: object,
	path: Path,
	selected: str,
) -> ParameterMap:
	"""Translate structured output options to workflow flags."""
...
class FourierConfig:
	"""Batch configuration for FourierPotential simulations."""

	version: str
	defaults: ParameterMap
	solver: ParameterMap = field(default_factory=dict)
	...
	def cases(self) -> list[FourierParams]:
		if self.case_overrides:
			raw_cases: list[ParameterMap] = []
			for override in self.case_overrides:
				case, case_solver = _merge_config_block(self.defaults, override, "solver")
				case, flat_solver = _split_keys(case, SOLVER_PARAM_KEYS)
				raw_cases.append(case | self.solver | flat_solver | case_solver)
		elif self.sweep:
			...
			for values in product(*[_as_list(self.sweep[key]) for key in keys]):
				...
				case, flat_solver = _split_keys(case, SOLVER_PARAM_KEYS)
				raw_cases.append(case | self.solver | flat_solver)
		else:
			case, flat_solver = _split_keys(self.defaults, SOLVER_PARAM_KEYS)
			raw_cases = [case | self.solver | flat_solver]
		normalized_cases = [
			_normalize_fourier_params(_expand_params(case))
...
class PotentialConfig:
	"""Configuration that builds a concrete GridPotential."""
	...
	sigma: float = 1
	k: int = 3
	mock: MockPotentialParams = field(
		default_factory=lambda: cast(MockPotentialParams, {})
	)

	def build(self) -> GridPotential:
		if self.type not in {"hdf5", "mock", "hdf5_or_mock"}:
			raise ConfigError(f"Invalid potential type {self.type!r}.")
		if (
			self.type in {"hdf5", "hdf5_or_mock"}
			and self.path is not None
			and self.path.exists()
		):
			target_shape = (
				None
				if self.nx is None and self.ny is None
...
class PotentialRunConfig:
	"""Configuration for a GridPotential simulation."""

	version: str
	potential: PotentialConfig
	trajectory: PotentialTrajectoryParams
	integration: PotentialIntegrationParams
	solver: SolverParams = field(default_factory=lambda: cast(SolverParams, {}))
	...
	def build_trajectory(self) -> Trajectory:
		return create_trajectory(
			self.trajectory.get("type", "gc"),
			rho=self.trajectory.get("rho", 0),
			eta=self.trajectory.get("eta", 0),
			n_trajectories=self.trajectory.get("Ntraj", 20),
			initialization=self.trajectory.get("init", "fixed"),
		)
...
def load_fourier_config(
	...
	path = (
		Path(path)
		if path is not None
		else config_path("fourier", config_surface, config_group, config_version)
	)
	...
	defaults = _apply_fourier_output(
		cast(ParameterMap, defaults),
		payload.get("output", {}),
		path,
		selected,
	)
	defaults, flat_solver = _split_keys(defaults, SOLVER_PARAM_KEYS)
	solver = payload.get("solver", {})
	if not isinstance(solver, dict):
		raise ConfigError(f"'solver' in {path}:{selected} must be an object.")
	...
	return FourierConfig(
		version=selected,
		defaults=defaults,
		solver=flat_solver | cast(ParameterMap, solver),
...
def load_potential_config(
	...
	path = (
		Path(path)
		if path is not None
		else config_path("potential", config_surface, config_group, config_version)
	)
	selected, payload = _version_payload(_read_config(path), version, path)
	...
	mock: MockPotentialParams = {
		"A": _as_float(
			raw_mock.get(
				"A",
				1 / _as_float(potential_payload.get("B", 1), "potential.B"),
			),
			"potential.mock.A",
		),
	...
	potential = PotentialConfig(
		...
		indx=(
			...
			else [_as_int(item, "potential.indx") for item in raw_indices]
		),
	...
	if not all(
		...
		raise ConfigError(
			f"'trajectory', 'integration', 'solver' and 'output' in "
			f"{path}:{selected} must be objects."
		)
	trajectory = cast(ParameterMap, trajectory_payload)
	integration = cast(ParameterMap, integration_payload)
	solver = cast(ParameterMap, solver_payload)
	output = cast(ParameterMap, output_payload)
	integration, flat_solver = _split_keys(integration, SOLVER_PARAM_KEYS)
	integration, flat_output = _split_keys(integration, OUTPUT_PARAM_KEYS)
	raw_trajectory_type = trajectory.get("type", "gc")
	trajectory_type = "fc" if raw_trajectory_type == "fo" else raw_trajectory_type
	if trajectory_type not in {"gc", "fc"}:
		raise ConfigError(f"Invalid trajectory type {raw_trajectory_type!r}.")
	...
	return PotentialRunConfig(
		...
		solver=cast(SolverParams, flat_solver | solver),
		output=cast(OutputParams, flat_output | output),
		output_dir=output_path_for_config(path),
		output_name=output_name,
	)
